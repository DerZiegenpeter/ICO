...
@export var neighbor_dist: float = 12.0		# max Abstand für Nachbar-Hexes
...
var hex_types: Array[String] = []				# "land", "ocean", "lake"
var spatial_hash: Dictionary = {}				# für schnelle Nachbar-Suche
...
func load_and_draw() -> void:
	...
	for i in data.size():
		...
		# Spatial Hash
		var key = _hash_key(center, cell)
		if not spatial_hash.has(key):
			spatial_hash[key] = []
		spatial_hash[key].append(hex_centers.size() - 1)
...
func _hash_key(pos: Vector3, cell: float) -> Vector2i:
...
func get_closest_hex_index(world_pos: Vector3) -> int:
	if hex_centers.is_empty():
		return -1
	var best_i = 0
	var best_d = world_pos.distance_squared_to(hex_centers[0])
...
func get_neighbors(index: int) -> Array[int]:
	...
	var cell = neighbor_dist * 1.5
	var center_key = _hash_key(pos, cell)
	
	for dx in range(-1, 2):
		for dz in range(-1, 2):
			var key = Vector2i(center_key.x + dx, center_key.y + dz)
			if not spatial_hash.has(key):
				continue
			for other_i in spatial_hash[key]:
				if other_i == index:
					continue
				if pos.distance_to(hex_centers[other_i]) <= neighbor_dist:
...
func find_path(from_pos: Vector3, to_pos: Vector3, unit_type: String) -> Array[Vector3]:
	...
	if not is_passable(goal_i, unit_type):
		print("Ziel nicht begehbar für ", unit_type)
		return []
	...
	print("Kein Pfad gefunden (", unit_type, ")")
	return []
