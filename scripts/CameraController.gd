...
@export var pan_sensitivity: float = 0.04
...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_WHEEL_UP:
			var step = zoom_speed * (_target.y * 0.03)
	...
	if event is InputEventMouseMotion and _panning:
		...
		_last_mouse = event.position
		var sens = pan_sensitivity * (_target.y * 0.045)
...
func _process(delta: float) -> void:
	...
	if dir != Vector3.ZERO:
		dir = dir.normalized()
		# Much faster when zoomed out
		var height_factor = clamp(_target.y * 0.035, 0.5, 14.0)
